    ...
    def split_at_timestamps(self, audio_path: str):
        ...
        for track in self.get_raw_tracks().items():
            ...
            if len(keys_who) > 0:
                ...
                chunk = "{0}{1}{2}".format(results, os.sep, stem)
                name = "{0}{1}{2}".format(chunk, os.sep, who)
                try:
                    os.makedirs(name)
                except OSError:
                    pass  # print("Creation of the directory %s failed" % name)
                else:
                    pass  # print("Successfully created the directory %s" % name)
                out_f = "{0}{1}{2}-{3}-{4}.wav" \
                    .format(name, os.sep, stem, str(floor(start)).zfill(7), str(floor(end)).zfill(7))
